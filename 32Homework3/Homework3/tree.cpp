...
//  32Homework3.4
//
//  Created by Shreya Donepudi on 13/02/17.
...
{
    if (n2 > n1)
        return 0;// this is not a valid scenario
    if (n2 <= 0)
        return 1;
    if (a1[0] == a2[0])
...
{
    if (n <= 1)
        return; // If n <= 1, do nothing
    // starting at the middle of the array to use as a "separator"
    // store the firstNotGreater, firstLess from the separate function
    // sort the left side of the array till a[firstNotGreater]
    //sort the array from a[firstLess] till the end
    int mid = (n+1)/2;
    int firstNotGreater, firstLess;
    separate(a, n, a[mid-1], firstNotGreater, firstLess);
